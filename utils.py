...
# utils.py
from db import get_session
from models import Event, Artist, Service, Format, Promoter, TourManager, Task, AuditLog
from sqlalchemy.orm import joinedload
from sqlalchemy.exc import SQLAlchemyError
...
def _safe_query_first(db, model, **filters):
...
def add_artist(name: str, role: str = "artist", phone: str = "", email: str = "", notes: str = "") -> Artist:
    ...
    try:
        ...
        db.commit()
        db.refresh(a)
        record_audit("artist", a.id, "create", {"name": a.name}, user="")
        return a
    except SQLAlchemyError:
        db.rollback()
        raise
...
def update_artist(artist_id: int, **fields) -> Optional[Artist]:
    ...
    try:
        ...
        db.commit()
        db.refresh(a)
        record_audit("artist", artist_id, "update", {"before": before, "after": fields}, user="")
        return a
    finally:
        db.close()
...
def delete_artist(artist_id: int) -> bool:
    ...
    try:
        ...
        db.delete(a)
        db.commit()
        record_audit("artist", artist_id, "delete", {"id": artist_id}, user="")
        return True
    finally:
        db.close()
...
def add_service(name: str, contact: str = "", phone: str = "", notes: str = "") -> Service:
    ...
    try:
        ...
        db.commit()
        db.refresh(s)
        record_audit("service", s.id, "create", {"name": s.name}, user="")
        return s
    finally:
        db.close()
...
def update_service(service_id: int, **fields) -> Optional[Service]:
    ...
    try:
        ...
        db.commit()
        db.refresh(s)
        record_audit("service", service_id, "update", {"before": before, "after": fields}, user="")
        return s
    finally:
        db.close()
...
def delete_service(service_id: int) -> bool:
    ...
    try:
        ...
        db.delete(s)
        db.commit()
        record_audit("service", service_id, "delete", {"id": service_id}, user="")
        return True
    finally:
        db.close()
...
def add_format(name: str, description: str = "", notes: str = "") -> Format:
    ...
    try:
        ...
        db.commit()
        db.refresh(f)
        record_audit("format", f.id, "create", {"name": f.name}, user="")
        return f
    finally:
        db.close()
...
def update_format(format_id: int, **fields) -> Optional[Format]:
    ...
    try:
        ...
        db.commit()
        db.refresh(f)
        record_audit("format", format_id, "update", {"before": before, "after": fields}, user="")
        return f
    finally:
        db.close()
...
def delete_format(format_id: int) -> bool:
    ...
    try:
        ...
        db.delete(f)
        db.commit()
        record_audit("format", format_id, "delete", {"id": format_id}, user="")
        return True
    finally:
        db.close()
...
def add_promoter(name: str, contact: str = "", phone: str = "", email: str = "", notes: str = "") -> Promoter:
    ...
    try:
        ...
        db.commit()
        db.refresh(p)
        record_audit("promoter", p.id, "create", {"name": p.name}, user="")
        return p
    finally:
        db.close()
...
def update_promoter(promoter_id: int, **fields) -> Optional[Promoter]:
    ...
    try:
        ...
        db.commit()
        db.refresh(p)
        record_audit("promoter", promoter_id, "update", {"before": before, "after": fields}, user="")
        return p
    finally:
        db.close()
...
def delete_promoter(promoter_id: int) -> bool:
    ...
    try:
        ...
        db.delete(p)
        db.commit()
        record_audit("promoter", promoter_id, "delete", {"id": promoter_id}, user="")
        return True
    finally:
        db.close()
...
def add_tour_manager(name: str, contact: str = "", phone: str = "", email: str = "", notes: str = "") -> TourManager:
    ...
    try:
        ...
        db.commit()
        db.refresh(t)
        record_audit("tour_manager", t.id, "create", {"name": t.name}, user="")
        return t
    finally:
        db.close()
...
def update_tour_manager(tm_id: int, **fields) -> Optional[TourManager]:
    ...
    try:
        ...
        db.commit()
        db.refresh(t)
        record_audit("tour_manager", tm_id, "update", {"before": before, "after": fields}, user="")
        return t
    finally:
        db.close()
...
def delete_tour_manager(tm_id: int) -> bool:
    ...
    try:
        ...
        db.delete(t)
        db.commit()
        record_audit("tour_manager", tm_id, "delete", {"id": tm_id}, user="")
        return True
    finally:
        db.close()
...
def assign_event(event_id: int, assignee: str, user: str = "") -> Optional[Event]:
    # assignment stored as a Task or in notes; here we append to notes for simplicity and record audit
    db = get_session()
    try:
        ev = db.query(Event).get(event_id)
        if not ev:
            return None
        before = {"notes": ev.notes}
        ev.notes = (ev.notes or "") + f"\n[ASSIGNED_TO:{assignee}]"
        db.commit()
        db.refresh(ev)
        record_audit("event", event_id, "assign", {"before": before, "after": {"assignee": assignee}}, user=user)
        return ev
    finally:
        db.close()
...
def update_task(task_id: int, **fields) -> Optional[Task]:
    ...
    try:
        ...
        db.commit()
        db.refresh(t)
        record_audit("task", task_id, "update", {"before": before, "after": fields}, user="")
        return t
    finally:
        db.close()
...
def create_tasks_from_template(event_id: int, template_name: str, assignee: str = "", due_date: Optional[date] = None, user: str = "") -> List[Task]:
    ...
    try:
        ...
        for t in created:
            db.refresh(t)
            record_audit("task", t.id, "create_from_template", {"template": template_name, "event_id": event_id}, user=user)
        return created
    finally:
        db.close()
...
def import_data_json(path: str, clear_existing: bool = False) -> None:
    ...
    try:
        ...
        db.commit()
        # events/tasks import can be implemented with id mapping if needed
    finally:
        db.close()
